...
def get_drug_info(drug_name):
    ...
    Returns:
        dict: A dictionary containing drug information or an error message.
...
def place_order(customer_name, drug_name):
    ...
    Args:
        ...
        drug_name (str): The name of the drug to order.
...
def lookup_order(order_id):
    ...
    Returns:
        dict: A dictionary containing order details or an error message.
    """
    order = ORDERS_DB["orders"].get(int(order_id)) # ORDERS_DB = {"orders": {1:{}}, "next_id": 1}
    if order:
        return {
            "order_id": order_id,
